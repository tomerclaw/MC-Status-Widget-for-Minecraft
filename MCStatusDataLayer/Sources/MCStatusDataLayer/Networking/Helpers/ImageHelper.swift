...
class ImageHelper {
    ...
    static func processCustomIcon(_ image: UIImage) -> Data? {
        ...
        if pixelWidth > 512 {
            let resized = cropped.resized(to: CGSize(width: 512, height: 512))
...
private extension UIImage {
    /// Center-crops the image to a square using the shorter dimension.
    func squareCropped() -> UIImage {
        ...
        // Multiply by scale to get pixel coordinates for cgImage
        let cropRect = CGRect(
            x: origin.x * scale,
            y: origin.y * scale,
            width: side * scale,
    ...
    /// Resizes the image to the given size using UIGraphicsImageRenderer.
    #if !os(watchOS)
